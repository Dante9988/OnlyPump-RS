  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (error.message?.includes('403') || error.message?.includes('Access forbidden')) {
        console.warn(
          `RPC rate limited, retrying in ${delay}ms... (attempt ${i + 1}/${maxRetries})`
        );
...
export function usePumpFunRedux() {
  ...
  const createToken = useCallback(
    async (request: CreateTokenRequest): Promise<TransactionResponse> => {
      ...
      if (!connection) {
        ...
        try {
          ...
          if (!connection) {
            throw new Error(
              'Failed to initialize Solana connection. Please refresh the page and try again.'
            );
          }
        } catch (error) {
      ...
      try {
        ...
        if (getNextPumpAddress.rejected.match(pumpAddressResult)) {
          throw new Error(
            'No pump addresses available. Please refresh the page to reload addresses.'
          );
        }
  ...
  const buyToken = useCallback(
    async (request: BuyTokenRequest): Promise<TransactionResponse> => {
      ...
      try {
        ...
        const { bondingCurveAccountInfo, bondingCurve, associatedUserAccountInfo } =
          await onlineSdk.fetchBuyState(mintPubkey, provider.publicKey);
  ...
  const sellToken = useCallback(
    async (request: SellTokenRequest): Promise<TransactionResponse> => {
      if (!connected || !publicKey || !signTransaction || !sendTransaction || !connection) {
        throw new Error('Wallet not connected or connection not initialized');
      }
  ...
  const calculateSellAmount = useCallback(
    async (mintAddress: string, tokenAmount: number): Promise<number> => {
      if (!connected || !publicKey || !connection) {
        return 0;
      }
  ...
  const fetchCreatorFees = useCallback(async (): Promise<{
    claimed: number;
    unclaimed: number;
    total: number;
  }> => {
    if (!connected || !publicKey || !connection) {
      console.log('fetchCreatorFees: Not ready', {
        connected,
        publicKey: !!publicKey,
        connection: !!connection,
      });
    ...
    try {
      ...
      console.log(
        '💰 Claimable vault balance (SDK, after rent):',
        ...
        if (response.ok) {
          ...
          for (const tx of transactions) {
            console.log(`🔍 Checking tx ${tx.signature?.slice(0, 8)}:`, {
              ...
              hasPumpFun: tx.accountData?.some(
            ...
            const isPumpFunTx = tx.accountData?.some(
              (acc: any) => acc.account === '6EF8rrecthR5Dkzon8Nwu78hRvfCKubJ14M5uBEwF6P'
            );

            if (isPumpFunTx && tx.nativeTransfers && tx.nativeTransfers.length > 0) {
              console.log(
                `🔍 Pump.fun tx ${tx.signature?.slice(0, 8)} native transfers:`,
                tx.nativeTransfers
              );
  ...
  const claimFees = useCallback(
    async (mintAddress?: string): Promise<TransactionResponse> => {
      if (!connected || !publicKey || !signTransaction || !sendTransaction || !connection) {
        throw new Error('Wallet not connected or connection not initialized');
      }
  ...
  const setSelectedTokenMint = useCallback(
    (mint: string | null) => {
