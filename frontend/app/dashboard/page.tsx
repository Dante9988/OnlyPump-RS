...
export default function CreatorDashboard() {
  ...
  const [feeData, setFeeData] = useState<{ claimed: number; unclaimed: number; total: number }>({
    claimed: 0,
    unclaimed: 0,
    total: 0,
  });
  ...
  useEffect(() => {
    const fetchTotalFees = async () => {
      if (!connected || !publicKey || !connection) {
        console.log('Dashboard: Not ready for fee fetching', {
          connected,
          publicKey: !!publicKey,
  ...
  const handleClaimFees = async (e?: React.MouseEvent) => {
    ...
    const loadingToast = toast.loading(
      `Claiming ${feeData.unclaimed.toFixed(4)} SOL in creator fees...`
    );
    ...
    try {
      ...
      toast.success(
        `✅ ${feeData.unclaimed.toFixed(4)} SOL claimed! TX: ${result.signature.slice(0, 8)}...`
      );
        ...
        <div className="max-w-5xl mx-auto px-4">
          ...
          <div className="flex items-center justify-between mb-12">
            <div>
              ...
              <p className="text-lg text-base-content/70">Claim your earnings from token trades</p>
            </div>
            <div className="flex items-center gap-3">
              <button
                onClick={() => {
                  const loadingToast = toast.loading('Refreshing...');
                  Promise.all([loadTokensFromStorage(), fetchDeployedTokens(), fetchCreatorFees()])
                    .then(([_, __, fees]) => {
                      setFeeData(fees);
                      toast.dismiss(loadingToast);
