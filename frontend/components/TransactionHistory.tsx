      ...
      <div className="card-body">
        ...
        <div className="space-y-2">
          {transactions.slice(0, isExpanded ? transactions.length : 5).map((tx, index) => (
            <div key={tx.signature || index} className="p-3 rounded-lg bg-base-300">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <div className="font-semibold">{getTransactionType(tx)}</div>
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs opacity-50">Slot: {tx.slot}</div>
                </div>
              </div>
                    ...
                    <div className="space-y-1">
                      {tx.instructions?.map((instruction: any, ixIndex: number) => (
                        <div key={ixIndex} className="text-xs font-mono bg-base-100 p-2 rounded">
                          ...
                          {instruction.data && <div>Data: {instruction.data.slice(0, 20)}...</div>}
