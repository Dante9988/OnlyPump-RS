    ...
    <div className="bg-base-200 p-4 rounded-lg mb-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
        <div>
          ...
          <p><strong>Endpoint:</strong> {connectionEndpoint || 'Not connected'}</p>
          <p><strong>Loading:</strong> {isLoading ? 'Yes' : 'No'}</p>
        </div>
        
        <div>
          <h4 className="font-semibold text-primary">Pump Addresses</h4>
          <p><strong>Count:</strong> {pumpAddresses.length}</p>
          <p><strong>Pool Size:</strong> {pumpAddressStats.pool_size}</p>
          <p><strong>Loading:</strong> {isLoadingPumpAddresses ? 'Yes' : 'No'}</p>
        </div>
        
        <div>
          <h4 className="font-semibold text-primary">Token Status</h4>
          <p><strong>Created Mint:</strong> {createdTokenMint || 'None'}</p>
        </div>
          ...
          <div className="max-h-32 overflow-y-auto">
            {pumpAddresses.slice(0, 3).map((addr, index) => (
              <div key={index} className="text-xs font-mono bg-base-300 p-2 rounded mb-1">
                <div><strong>Public:</strong> {addr.public_key}</div>
                <div><strong>Private:</strong> {addr.private_key.substring(0, 20)}...</div>
              </div>
            ))}
