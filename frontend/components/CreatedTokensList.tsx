...
export default function CreatedTokensList({ tokens, deployedTokens, isLoadingDeployedTokens, onSelectToken, selectedToken }: CreatedTokensListProps) {
  ...
  return (
    <div className="card bg-base-200 shadow-xl">
        ...
        <div className="space-y-2">
          {(showDeployed ? deployedTokens : tokens).slice(0, isExpanded ? (showDeployed ? deployedTokens : tokens).length : 3).map((token) => (
            <div
              key={token.mint}
          ...
          {(showDeployed ? deployedTokens : tokens).length > 3 && (
            ...
            >
              {isExpanded ? 'Show Less' : `Show All (${(showDeployed ? deployedTokens : tokens).length})`}
            </button>
          )}
        </div>
      </div>
